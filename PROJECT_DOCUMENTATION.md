...
UI Design: CSS-like styling in Flutter with BackdropFilter (Glassmorphism).

3. Core Architecture
The project follows a modular architecture to ensure maintainability and scalability:
A. Data Models (lib/model)
Assessment: Represents the high-level assessment entity (e.g. Science Presentation).
AssessmentDetail: Represents specific criteria within an assessment (e.g. Comment option, Assessment Marking).
Student: Represents all the students.
...
Logic Implementation: If a teacher provides a specific finding/comment, the system automatically sets the comment yes per project rules.
...
Zero Downtime: Works perfectly regardless of internet availability.
High Performance: Instant UI response times due to local database priority.
Data Integrity: Background synchronization ensures that offline work is never lost.
Professionalism: A modern, high-end look that meets current industry standards for mobile application design.
